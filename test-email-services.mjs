...
// Recipient emails (where we want to receive the test)
const RECIPIENT_EMAILS = ['dealappseo@gmail.com', 'sean@ccanaheim.com'];
  ...
  try {
    ...
    for (const email of RECIPIENT_EMAILS) {
      ...
      const data = {
        ...
        html: '<div style="color: #333; font-family: Arial, sans-serif;"><h2>HyperDAG Mailgun Test</h2><p>This is a test email sent via Mailgun to verify email delivery.</p><p>If you received this email, Mailgun is working!</p><p>Timestamp: ' + new Date().toISOString() + '</p></div>'
      };
    
      try {
        console.log(`Sending test email via Mailgun to ${email}...`);
    ...
    console.log(success ? '✅ Mailgun test emails sent successfully!' : '⚠️ Some emails failed to send');
    return success;
  } catch (error) {
  ...
  try {
    ...
    for (const email of RECIPIENT_EMAILS) {
      ...
      const msg = {
        ...
        html: '<div style="color: #333; font-family: Arial, sans-serif;"><h2>HyperDAG SendGrid Test</h2><p>This is a test email sent via SendGrid to verify email delivery.</p><p>If you received this email, SendGrid is working!</p><p>Timestamp: ' + new Date().toISOString() + '</p></div>'
      };
    
      try {
        console.log(`Sending test email via SendGrid to ${email}...`);
        const result = await sgMail.send(msg);
        console.log(`SendGrid API response for ${email}:`, JSON.stringify(result, null, 2));
      } catch (error) {
        console.error(`Failed to send to ${email}:`, error);
        success = false;
      }
    ...
    console.log(success ? '✅ SendGrid test emails sent successfully!' : '⚠️ Some SendGrid emails failed to send');
    return success;
  } catch (error) {
...
async function main() {
  ...
  console.log('Recipient Emails: ' + RECIPIENT_EMAILS.join(', '));
  ...
  if (mailgunResult || sendgridResult) {
    ...
    console.log('If you don\'t see them, check your spam folder.');
  } else {
