...
async function importRealTasks() {
  ...
  for (const task of REAL_TRINITY_TASKS) {
    ...
    if (error) {
      ...
      console.log(`✅ [P${task.priority} ${task.task_type}] ${task.title}`);
      successCount++;
    }
  }

  console.log('\n' + '='.repeat(60));
  console.log(`✅ Imported: ${successCount} tasks | ⏭️  Skipped: ${skipCount} tasks\n`);
  ...
  console.log('📊 Real Tasks by Priority:');
  [5,4,3,2,1].forEach(p => {
    if (priorityCount[p]) {
      console.log(`  Priority ${p}: ${priorityCount[p]} tasks`);
    }
  });
  ...
  console.log('\n🎯 Trinity Symphony ready to dogfood itself!');
}
