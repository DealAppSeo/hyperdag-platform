...
function createDnsInstructions() {
  ...
  console.log('   Type: A');
  console.log('   Name: @');
  console.log('   Value: 159.203.161.8'); // Replit's IP address
  ...
  console.log('\nAlternatively, you can set both domains to the same A record:');
  console.log('   Type: A (for both domains)');
  console.log('   Name: @ (for hyperdag.org) and www (for www.hyperdag.org)');
  console.log('   Value: 159.203.161.8');
  
  console.log('\nAlternative method through Cloudflare (recommended):');
  console.log('1. Add both domains (hyperdag.org and www.hyperdag.org) to Cloudflare');
  console.log('2. Create A records pointing to 159.203.161.8');
  console.log('3. Enable the orange cloud (proxy) for both domains');
  console.log('4. Set SSL/TLS encryption mode to Full');
...
async function main() {
  ...
  if (success) {
    ...
    console.log('\nConsider using Cloudflare as your DNS provider with proxy enabled for better security and faster propagation.');
  } else {
